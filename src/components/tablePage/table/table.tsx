    ...
    return (
        <table className={styles.table}>
            ...
            <tr>
                ...
                <th className={styles.columnHead}>
                    {country === 'USA' ? <div><span>Доля</span> <span>%</span></div>
                        :
                        'Сектор'}
                </th>
                <th className={styles.columnHead}>
                    {country === 'USA' ? <div><span>Дивиденды</span><span>%</span></div>
                        :
                        'Отрасль'}
                </th>
                <th className={styles.columnHead}>
            ...
            {state && state.slice(firstCompany, firstCompany + 15).map((company, index) => {
                return <tr key={`${company && (company['компания'])} ${index}`} className={country === 'China' ? styles.indicatorFontSize : ''}>
                    ...
                    <td className={styles.companyIndicator}>
                        {company['доля'] ? company['доля'].slice(0, -1) : company['Сектор'] ? company['Сектор'] : '-'}
                    </td>
                    <td className={styles.companyIndicator}
